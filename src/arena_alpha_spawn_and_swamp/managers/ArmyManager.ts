...
export class ArmyManager extends Manager {
    ...
    constructor(spawn: StructureSpawn, enemySpawn: StructureSpawn, midGameEconomyManager: MidGameEconomyManager) {
        super()
    ...
    buildArmy(): boolean {
        if (this.hunter) {
            var newCreep = attempSpawn(this.spawn, KITE_TEMPLATE);
            if (newCreep) {
                this.army.push(newCreep);
            }
            return true;
        } else {
            this.hunter = attempSpawn(this.spawn, HUNTER_TEMPLATE);
        }
        return false;
    ...
    tickArmy(gatherer: Creep | undefined) {
        ...
        for (var creep of this.army) {
            this.tickArmyCreep(creep, enemyArmy, enemyWorkers, this.enemySpawn, this.army.length <= 4);
        }
    }
